...
#     if selected_goal:
#         # Find the selected goal's index
#         selected_idx = next((i for i, goal in enumerate(st.session_state.goals) if goal["Goal"] == selected_goal), None)
        
#         if selected_idx is not None:
#             st.write(f"### Updating: {selected_goal}")
#             status_options = ["Not Started", "In Progress", "Completed"]
#             new_status = st.selectbox("Update Status", status_options, index=status_options.index(st.session_state.goals[selected_idx]["Status"]), key="status_update")
